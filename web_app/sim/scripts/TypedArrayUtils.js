...
THREE.TypedArrayUtils.quicksortIP = function ( arr, indices, eleSize, orderElement ) {
	...
	var i, j, swap = new Float32Array( eleSize ), temp = new Float32Array( eleSize ), itemp, iswap;
 ...
 */
 ...
 THREE.TypedArrayUtils.Kdtree = function ( points, indices, metric, eleSize ) {
	...
	function buildTree( points, indices, depth, parent, pos ) {
		...
		THREE.TypedArrayUtils.quicksortIP( points, indices, eleSize, dim );
		
		median = Math.floor( plength / 2 );
		
		node = new self.Node( getPointSet( points, median ), depth, parent, median + pos  );
		node.left = buildTree( points.subarray( 0, median * eleSize ), indices.subarray(0, median), depth + 1, node, pos );
		node.right = buildTree( points.subarray( ( median + 1 ) * eleSize, points.length ), indices.subarray(median + 1, indices.length), depth + 1, node, pos + median + 1 );

		return node;
	
	}

	this.root = buildTree( points, indices, 0, null, 0 );
